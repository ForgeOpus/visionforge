 ...
 */
export function useNodeSpecs(options = {}) {
    ...
    const getSpec = useCallback((nodeType) => {
        return specs.find(spec => spec.type === nodeType);
    }, [specs]);
    const renderCode = useCallback(async (nodeType, config) => {
        try {
 ...
 */
export function useNodeSpec(nodeType, framework = 'pytorch') {
