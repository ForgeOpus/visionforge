...
import { NodeValidationState, } from './validation';
...
export const useModelBuilderStore = create((set, get) => ({
    nodes: [],
    edges: [],
    selectedNodeId: null,
    selectedEdgeId: null,
    ...
    addNode: (node) => {
        ...
        get().trackRecentlyUsedNode(node.data.blockType);
    ...
    validateConnection: (connection) => {
        ...
        const targetNodeDef = getNodeDefinition(targetNode.data.blockType, BackendFramework.PyTorch);
        const sourceNodeDef = getNodeDefinition(sourceNode.data.blockType, BackendFramework.PyTorch);
        if (!targetNodeDef || !sourceNodeDef)
            return false;
        ...
        if (!isMergeNode) {
            const handleOccupied = edges.some(e => e.target === connection.target &&
        ...
        if (targetNode.data.blockType === 'loss') {
            ...
            if (totalConnectionsAfter > requiredPorts.length) {
                ...
                console.error(`Loss function "${lossType}" only accepts ${requiredPorts.length} inputs ` +
            ...
            if (node.data.blockType === 'loss') {
                ...
                if (lossNodeDef?.getInputPorts) {
                    ...
                    if (incomingEdges.length !== requiredPorts.length) {
                        errors.push({
                            nodeId: node.id,
                            message: `Loss function "${node.data.config.loss_type || 'cross_entropy'}" requires ${requiredPorts.length} inputs (${requiredPorts.map((p) => p.label).join(', ')}), but has ${incomingEdges.length}`,
                            type: 'error'
                        });
                    }
                    else {
                        // Check that all required ports are filled (handle-aware)
                        const connectedHandles = new Set(incomingEdges.map(e => e.targetHandle || 'default'));
                        const missingPorts = requiredPorts.filter((p) => !connectedHandles.has(p.id));
    ...
    inferDimensions: () => {
        ...
        const getIncomingEdges = (nodeId) => edges.filter((e) => e.target === nodeId);
                    ...
                    if ((node.data.blockType === 'concat' || node.data.blockType === 'add') && incomingEdges.length > 1) {
                        ...
                        if (sourceNode?.data.outputShape) {
                            ...
                            if (nodeDef) {
                                ...
                                node.data.shapeStatus = {
                                    state,
                                    inputShapes: [sourceNode.data.outputShape],
                                    outputShape: outputShape,
                                    timestamp: Date.now()
                                };
                            }
        ...
        const standaloneNodes = updatedNodes.filter((n) => !visited.has(n.id) &&
