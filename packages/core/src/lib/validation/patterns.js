 ...
 */
export function rankRange(min, max) {
    return {
        rank: { min, max },
 ...
 */
export function rankOneOf(ranks) {
    return {
        rank: { oneOf: ranks },
 ...
 */
export function axis(role, options) {
    return {
        role,
        ...options,
    };
 ...
 * Batch axis (B)
 */
export function batchAxis(options) {
    return {
        role: 'batch',
 ...
 * Channel axis (C)
 */
export function channelAxis(value) {
    return {
        role: 'channels',
 ...
 * Feature axis (F)
 */
export function featureAxis(value) {
    return {
        role: 'features',
 ...
 */
export function timeAxis(options) {
    return {
        role: 'time',
        symbol: SYMBOLS.TIME,
        flexible: true,
 ...
 * Height axis (H)
 */
export function heightAxis(value) {
    return {
        role: 'height',
 ...
 * Width axis (W)
 */
export function widthAxis(value) {
    return {
        role: 'width',
 ...
 */
export function endsWith(axisSpec) {
    return {
        endsWith: axisSpec,
 ...
 */
export function startsWithBatch() {
    return {
 ...
 */
export function broadcastableTo(targetPattern) {
    return {
 ...
 */
export function projectionInput(inFeatures) {
    return {
        rank: { min: 2 },
 ...
 */
export function spatialInput(inChannels) {
    return {
        rank: { min: 4 },
        axes: [
            batchAxis(),
 ...
 */
export function sequenceInput(seqLen, features) {
    return {
        rank: { exact: 3 },
        axes: [
            batchAxis(),
 ...
 */
export function classificationOutput(numClasses) {
    return {
        rank: { exact: 2 },
        axes: [
            batchAxis(),
 ...
 */
export function concatMerge(axis = 1) {
    return {
 ...
 * Stack merge pattern
 */
export function stackMerge(axis = 0) {
    return {
        wildcard: true,
        mergeRule: {
            mode: 'stack',
 ...
 */
export function mseLossPattern() {
    return {
        prediction: wildcard(),
 ...
 */
export function crossEntropyPattern() {
    return {
        prediction: {
 ...
 */
export function bceLossPattern() {
    return {
        prediction: wildcard(),
...
export function combinePatterns(...patterns) {
    ...
    for (const pattern of patterns) {
        if (pattern.rank)
            combined.rank = pattern.rank;
        if (pattern.axes)
            combined.axes = pattern.axes;
        if (pattern.endsWith)
            combined.endsWith = pattern.endsWith;
        if (pattern.startsWith)
            combined.startsWith = pattern.startsWith;
        if (pattern.wildcard)
            combined.wildcard = pattern.wildcard;
        if (pattern.broadcastableTo)
            combined.broadcastableTo = pattern.broadcastableTo;
        if (pattern.mergeRule)
            combined.mergeRule = pattern.mergeRule;
        if (pattern.preserveLeading !== undefined)
            combined.preserveLeading = pattern.preserveLeading;
