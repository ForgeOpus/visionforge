 ...
 */
export function isNumeric(dim: DimensionValue): dim is number {
 ...
 */
export function isSymbolic(dim: DimensionValue): dim is string {
 ...
 */
export function getRank(shape: TensorShape): number {
 ...
 */
export function isFullyNumeric(shape: TensorShape): boolean {
 ...
 */
export function hasSymbolicDims(shape: TensorShape): boolean {
 ...
 */
export function getLastDim(shape: TensorShape): DimensionValue | undefined {
 ...
 */
export function getFirstDim(shape: TensorShape): DimensionValue | undefined {
  return shape.dims[0];
}
 ...
 */
export function matchRank(shape: TensorShape, rankSpec: number | RankSpec): MatchResult {
...
export function areFeaturesCompatible(
  sourceDim: DimensionValue,
  targetDim: DimensionValue
): MatchResult {
...
export function flattenShape(
  ...
  if (dimsToFlatten.every(isNumeric)) {
    const product = (dimsToFlatten as number[]).reduce((a, b) => a * b, 1);
    return {
      dims: [
 ...
 */
export function autoFlattenForProjection(shape: TensorShape): TensorShape {
  if (getRank(shape) <= 2) {
    return shape;
  }
 ...
 */
export function canConcatenate(
  shapes: TensorShape[],
  axis: number = 1
): MatchResult {
  if (shapes.length < 2) {
 ...
 */
export function canAdd(shapes: TensorShape[]): MatchResult {
  if (shapes.length < 2) {
    return { matches: true, code: ValidationCode.OK, message: 'Single input' };
  }
 ...
 */
export function computeConcatShape(
  shapes: TensorShape[],
  axis: number = 1
): TensorShape | null {
