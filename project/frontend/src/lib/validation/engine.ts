...
export class ValidationEngine {
   ...
   */
  validateConnection(
    sourceShape: TensorShape | undefined,
    targetPattern: ShapePattern,
    ...
    if (targetPattern.endsWith?.numeric !== undefined) {
      ...
      if (lastDim !== undefined) {
        const featureResult = areFeaturesCompatible(
          lastDim,
          targetPattern.endsWith.numeric
        );
    ...
    let normalizedShape: ExtendedTensorShape = {
    ...
    if (
      context.targetConfig.auto_flatten === true &&
      targetPattern.rank &&
  ...
  inferOutputShape(
    ...
    config: BlockConfig,
    inputPattern: ShapePattern,
    outputPattern?: ShapePattern,
    ...
    const outputShape = this.computeNodeOutput(
      nodeType,
      effectiveInput,
      config,
      outputPattern
    ...
    return {
      shape: outputShape as ExtendedTensorShape,
      status,
      code: ValidationCode.OK,
  ...
  negotiateMerge(
    ...
    if (inputShapes.length === 1) {
      return {
        shape: inputShapes[0] as ExtendedTensorShape,
        ok: true,
        code: ValidationCode.OK,
  ...
  createShapeStatus(
    ...
    return {
      ...
      inputShapes: inputShapes as ExtendedTensorShape[],
      outputShape: outputShape as ExtendedTensorShape | undefined,
      inputValidation,
      outputInference,
      timestamp: Date.now(),
    };
  ...
  private computeLinearOutput(
    ...
    const outFeatures = config.out_features as number;
    if (!outFeatures || outFeatures <= 0) {
      return null;
    }

    // Output has same leading dimensions, with last dimension replaced
    const dims: DimensionValue[] = [...inputShape.dims.slice(0, -1), outFeatures];
  ...
  private computeConv2DOutput(
    ...
    if (getRank(inputShape) < 4) return null;

    const outChannels = config.out_channels as number;
    const kernelSize = config.kernel_size as number | number[];
    const stride = (config.stride as number | number[]) || 1;
    const padding = (config.padding as number | number[]) || 0;
    const dilation = (config.dilation as number | number[]) || 1;

    if (!outChannels || !kernelSize) return null;
    ...
    let outH: DimensionValue;
    let outW: DimensionValue;
  ...
  private computeFlattenOutput(
    ...
    const endDim = (config.end_dim as number) ?? -1;
  ...
  private computePoolOutput(
    ...
    const kernelSize = config.kernel_size as number | number[];
    const stride = (config.stride as number | number[]) || kernelSize;
    const padding = (config.padding as number | number[]) || 0;

    if (!kernelSize) return null;
  ...
  private negotiateConcat(
    ...
    return {
      shape: shape as ExtendedTensorShape,
      ok: true,
      code: ValidationCode.OK,
  ...
  private negotiateAdd(inputShapes: TensorShape[]): MergeResult {
  ...
  private isConfigComplete(config: BlockConfig): boolean {
    // Basic check - could be enhanced with schema validation
    return Object.keys(config).length > 0;
  }

  private getActionHint(code: ValidationCode, pattern: ShapePattern): string {
    switch (code) {
