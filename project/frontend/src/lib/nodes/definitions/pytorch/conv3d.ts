/**
 * PyTorch Conv3D Layer Node Definition
 */
...
export class Conv3DNode extends NodeDefinition {
  ...
  computeOutputShape(inputShape: TensorShape | undefined, config: BlockConfig): TensorShape | undefined {
    ...
    if (inputShape.dims.length !== 5) {
      return undefined
    }

    const [batch, _, depth, height, width] = inputShape.dims as number[]
    ...
    const outDepth = Math.floor((depth + 2 * padding - dilation * (kernel - 1) - 1) / stride) + 1
    const outHeight = Math.floor((height + 2 * padding - dilation * (kernel - 1) - 1) / stride) + 1
    const outWidth = Math.floor((width + 2 * padding - dilation * (kernel - 1) - 1) / stride) + 1

    return {
      dims: [batch, config.out_channels as number, outDepth, outHeight, outWidth],
      description: `Conv3D(${config.out_channels})`
    }
  }
  ...
  validateIncomingConnection(
    ...
    return this.validateDimensions(sourceOutputShape, {
      dims: 5,
      description: '[batch, channels, depth, height, width]'
    })
  }
}
