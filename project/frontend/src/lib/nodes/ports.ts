...
export type PortSemantic = 
  | 'data'           // Regular data flow (activations, features)
  | 'labels'         // Ground truth labels
  | 'loss'           // Loss value output
  | 'predictions'    // Model predictions
  | 'anchor'         // Anchor for triplet loss
  | 'positive'       // Positive example for triplet loss
  | 'negative'       // Negative example for triplet loss
  | 'input1'         // Generic first input
  | 'input2'         // Generic second input
...
export interface PortDefinition {
  ...
  required: boolean
  description: string
  acceptsMultiple?: boolean  // For inputs that can receive multiple connections
}
 ...
 */
export function arePortsCompatible(source: PortDefinition, target: PortDefinition): boolean {
